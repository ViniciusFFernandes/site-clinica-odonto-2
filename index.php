<?php
// index.php — Dr(a). Nome Sobrenome (conteúdo em config/clinica.php)
...
<head>
  ...
  <meta name="theme-color" content="#4a5a48" />
...
<body class="min-h-screen bg-background font-sans text-foreground antialiased">
  ...
  <header id="siteHeader" class="fixed inset-x-0 top-0 z-50 transition-all duration-300 py-5">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 lg:px-8">
      <a href="#topo" class="group flex min-w-0 items-center gap-3">
        <img src="<?= e($cfg['logo']) ?>" alt="<?= e($cfg['clinic']) ?>" class="size-11 shrink-0" />
  ...
  <main id="topo">
    ...
    <section class="relative overflow-hidden" style="background: var(--gradient-deep)">
      <div aria-hidden="true" class="pointer-events-none absolute -top-32 -right-24 size-[40rem] rounded-full bg-rose/15 blur-3xl"></div>
      <div aria-hidden="true" class="pointer-events-none absolute -bottom-40 -left-32 size-[34rem] rounded-full bg-sage/30 blur-3xl"></div>
      <div aria-hidden="true" class="pointer-events-none absolute inset-0 opacity-[0.05]" style="background-image: radial-gradient(circle at 1px 1px, #fff 1px, transparent 0); background-size: 34px 34px;"></div>
      ...
      <div class="relative mx-auto grid max-w-7xl items-center gap-12 px-5 pt-36 pb-20 lg:grid-cols-12 lg:gap-8 lg:px-8 lg:pt-44 lg:pb-28">
        <div class="reveal lg:col-span-6">
          <span class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-semibold tracking-wide text-rose uppercase backdrop-blur">
            <span class="relative flex size-2">
              <span class="absolute inline-flex size-full animate-ping rounded-full bg-rose opacity-75"></span>
              <span class="relative inline-flex size-2 rounded-full bg-rose"></span>
            </span>
            Dentista em Cidade Exemplo · BA
          </span>

          <h1 class="mt-7 font-display text-4xl leading-[1.08] font-500 tracking-tight text-white text-balance sm:text-5xl lg:text-[3.4rem]">
            <?= e($cfg['hero_titulo_1']) ?> <span class="italic font-400 text-rose"><?= e($cfg['hero_titulo_2']) ?></span>
          </h1>
          ...
          <div class="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <a href="#tratamentos" class="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 bg-white/5 px-7 py-4 text-base font-semibold text-white backdrop-blur transition-colors hover:bg-white/10">
              <?= icon('arrow-right', 'size-5') ?>Conhecer tratamentos
            </a>
          </div>
          ...
          <div class="mt-10 inline-flex items-center gap-4 rounded-2xl border border-white/12 bg-white/5 px-5 py-4 backdrop-blur">
            <div class="text-center">
              ...
              <div class="mt-1 flex gap-0.5 text-rose"><?php for($k=0;$k<5;$k++) echo icon('star','size-3.5 fill-rose'); ?></div>
            </div>
            <div class="h-10 w-px bg-white/15"></div>
            <div class="text-sm leading-snug text-white/70">
              <div class="font-semibold text-white">5.0 no Google</div>
              <div><?= e($cfg['reviews']) ?> avaliações</div>
            </div>
          </div>
        </div>
        ...
        <div class="reveal relative lg:col-span-6 lg:pl-10" style="transition-delay:140ms">
          <div class="relative">
            <div class="overflow-hidden rounded-[2.75rem] rounded-tr-[6rem] border border-white/10 shadow-[var(--shadow-lift)]">
              <img src="img/hero-clinica.svg" alt="Dr(a). Nome Sobrenome atendendo uma paciente em consultório moderno e iluminado" width="1000" height="1150" class="h-full w-full object-cover" />
            </div>
            <div class="absolute -bottom-6 -left-4 flex items-center gap-3 rounded-2xl border border-border bg-background/95 px-5 py-4 shadow-[var(--shadow-lift)] backdrop-blur sm:-left-8">
              <span class="grid size-11 shrink-0 place-items-center rounded-xl text-accent-foreground" style="background: var(--gradient-rose)"><?= icon('heart-handshake', 'size-5') ?></span>
              <span class="min-w-0">
                <span class="block text-sm font-semibold text-primary">Atendimento humanizado</span>
                <span class="block text-xs text-muted-foreground">Cuidado em cada detalhe</span>
              </span>
            </div>
            <div class="absolute -top-5 right-2 hidden items-center gap-2 rounded-full border border-border bg-background/95 px-4 py-2.5 text-xs font-semibold text-primary shadow-[var(--shadow-soft)] backdrop-blur sm:flex">
              <?= icon('sparkles', 'size-4 text-accent') ?>Estética & saúde do sorriso
            </div>
          </div>
        </div>
      </div>

      <a href="#diferenciais" class="relative mx-auto mb-8 hidden w-fit items-center gap-2 px-8 text-xs font-medium tracking-widest text-white/50 uppercase transition-colors hover:text-rose lg:flex">
    ...
    <section id="diferenciais" class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="grid gap-8 lg:grid-cols-12 lg:items-end">
        <div class="reveal lg:col-span-5">
          ...
          <h2 class="mt-4 font-display text-4xl font-500 tracking-tight text-primary text-balance sm:text-5xl">Um cuidado pensado <span class="italic font-400 text-accent">para você</span></h2>
        </div>
        <p class="reveal text-lg leading-relaxed text-muted-foreground lg:col-span-6 lg:col-start-7" style="transition-delay:80ms">
          Cada detalhe do atendimento existe para transmitir delicadeza, segurança e confiança — antes, durante e depois do seu tratamento.
        </p>
      </div>
      ...
      <div class="mt-14 grid gap-px overflow-hidden rounded-3xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
        <?php foreach ($cfg['diferenciais'] as $i => $r): ?>
          <div class="reveal bg-card" style="transition-delay:<?= ($i % 3) * 80 ?>ms">
            <article class="group flex h-full flex-col gap-5 p-8 transition-colors hover:bg-secondary/60 lg:p-10">
              <div class="flex items-center justify-between">
                <span class="grid size-14 place-items-center rounded-2xl bg-accent-soft text-accent transition-all duration-300 group-hover:scale-110 group-hover:bg-accent group-hover:text-accent-foreground"><?= icon($r['icon'], 'size-7') ?></span>
    ...
    <!-- Institucional (emocional) -->
    <section id="sobre" class="relative overflow-hidden bg-secondary/60 py-20 lg:py-28">
      <div aria-hidden="true" class="pointer-events-none absolute -top-24 -left-24 size-[30rem] rounded-full bg-accent-soft/50 blur-3xl"></div>
      <div class="relative mx-auto max-w-7xl px-5 lg:px-8">
        <div class="grid items-center gap-12 lg:grid-cols-12 lg:gap-16">
          <div class="reveal lg:col-span-5">
            <div class="relative">
              <div class="overflow-hidden rounded-[2.75rem] rounded-bl-[6rem] shadow-[var(--shadow-lift)]">
              ...
              <div class="absolute -right-4 -bottom-6 rounded-2xl border border-border bg-background px-6 py-5 text-center shadow-[var(--shadow-lift)] sm:-right-8">
                <div class="font-display text-3xl font-600 text-accent"><?= e($cfg['rating']) ?><span class="text-rose">★</span></div>
          ...
          <div class="reveal lg:col-span-6 lg:col-start-7" style="transition-delay:100ms">
            ...
            <h2 class="mt-4 font-display text-4xl font-500 tracking-tight text-primary text-balance sm:text-5xl"><?= e($cfg['sobre_titulo']) ?></h2>
    ...
    <section id="tratamentos" class="relative overflow-hidden py-20 lg:py-28">
      <div aria-hidden="true" class="pointer-events-none absolute -top-24 right-0 size-[30rem] rounded-full bg-accent-soft/40 blur-3xl"></div>
      <div class="relative mx-auto max-w-7xl px-5 lg:px-8">
        <div class="reveal mx-auto max-w-3xl text-center">
          <span class="text-xs font-semibold tracking-[0.2em] text-accent uppercase">Tratamentos</span>
          <h2 class="mt-4 font-display text-4xl font-500 tracking-tight text-primary text-balance sm:text-5xl">Cuidados pensados para <span class="italic font-400 text-accent">cada sorriso</span></h2>
          <p class="mt-5 text-lg text-muted-foreground">Da estética à saúde bucal, um portfólio completo conduzido com técnica, delicadeza e planejamento individual.</p>
        </div>

        <!-- Chips de navegação -->
        <div class="reveal mt-12 flex flex-wrap justify-center gap-2.5" role="tablist" aria-label="Tratamentos">
          <?php foreach ($cfg['especialidades'] as $i => $s): ?>
            <button type="button" role="tab" data-esp-tab="<?= e($s['key']) ?>" aria-selected="<?= $i === 0 ? 'true' : 'false' ?>"
              class="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2.5 text-sm font-semibold text-primary transition-all hover:border-accent/50 <?= $i === 0 ? 'esp-tab-active' : '' ?>">
              <span class="esp-tab-icon text-accent"><?= icon($s['icon'], 'size-4') ?></span>
              <?= e($s['title']) ?>
            </button>
          <?php endforeach; ?>
        </div>
        ...
        <div class="mt-12">
          <?php foreach ($cfg['especialidades'] as $i => $s): ?>
            <div data-esp-panel="<?= e($s['key']) ?>" role="tabpanel" class="<?= $i === 0 ? '' : 'hidden' ?>">
              <div class="grid items-stretch gap-8 overflow-hidden rounded-[2.5rem] border border-border bg-card shadow-[var(--shadow-soft)] lg:grid-cols-2">
                <!-- Imagem -->
                <div class="relative min-h-[280px] overflow-hidden <?= $i % 2 === 1 ? 'lg:order-2' : '' ?>">
                  <img src="<?= e($s['img']) ?>" alt="<?= e($s['title']) ?> — <?= e($cfg['clinic']) ?>" loading="lazy" width="900" height="700" class="h-full w-full object-cover" />
                  <?php if (!empty($s['featured'])): ?>
                    <span class="absolute top-5 left-5 inline-flex items-center gap-1.5 rounded-full bg-background/90 px-3.5 py-1.5 text-xs font-bold tracking-wide text-accent uppercase shadow-[var(--shadow-soft)] backdrop-blur"><?= icon('sparkles', 'size-3.5') ?>Mais procurado</span>
                  <?php endif; ?>
                </div>
                <!-- Conteúdo -->
                <div class="flex flex-col justify-center p-8 lg:p-12">
                  <span class="inline-flex size-14 items-center justify-center rounded-2xl bg-accent-soft text-accent"><?= icon($s['icon'], 'size-7') ?></span>
                  <h3 class="mt-6 font-display text-3xl font-600 tracking-tight text-primary"><?= e($s['title']) ?></h3>
                  <p class="mt-4 leading-relaxed text-muted-foreground"><?= e($s['desc']) ?></p>
                  <ul class="mt-6 grid gap-2.5 sm:grid-cols-2">
                    <?php foreach ($s['benef'] as $b): ?>
                      <li class="flex items-start gap-2.5 text-sm font-medium text-primary">
                        <span class="mt-0.5 grid size-5 shrink-0 place-items-center rounded-full bg-accent text-accent-foreground"><?= icon('check', 'size-3.5') ?></span>
                        <?= e($b) ?>
                      </li>
                    <?php endforeach; ?>
                  </ul>
                  <a href="<?= e(whats($cfg, 'Olá, Dr(a). Nome Sobrenome! Gostaria de saber mais sobre ' . $s['title'] . '.')) ?>" target="_blank" rel="noopener noreferrer" class="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-accent px-6 py-3.5 text-sm font-semibold text-accent-foreground shadow-[var(--shadow-soft)] transition-transform hover:scale-[1.03]">
                    <?= icon('message-circle', 'size-4') ?>Agendar <?= e($s['title']) ?>
                  </a>
                </div>
              </div>
            </div>
    ...
    <!-- Experiência do paciente (linha do tempo) -->
    <section id="processo" class="relative overflow-hidden py-20 lg:py-28" style="background: var(--gradient-deep)">
      <div aria-hidden="true" class="pointer-events-none absolute -bottom-40 -right-32 size-[36rem] rounded-full bg-rose/10 blur-3xl"></div>
      <div class="relative mx-auto max-w-7xl px-5 lg:px-8">
        <div class="reveal mx-auto max-w-2xl text-center">
          <span class="text-xs font-semibold tracking-[0.2em] text-rose uppercase">Experiência do paciente</span>
          <h2 class="mt-4 font-display text-4xl font-500 tracking-tight text-white text-balance sm:text-5xl">Do primeiro contato ao <span class="italic font-400 text-rose">novo sorriso</span></h2>
          <p class="mt-5 text-lg text-white/60">Um caminho leve e tranquilo, com você no centro de cada decisão.</p>
        </div>

        <ol class="mt-16 grid gap-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 md:gap-4">
          <?php foreach ($cfg['jornada'] as $i => $p): ?>
            <li class="reveal group relative" style="transition-delay:<?= $i * 80 ?>ms">
              <?php if ($i < count($cfg['jornada']) - 1): ?>
                <span aria-hidden="true" class="absolute top-7 left-[calc(50%+2.5rem)] hidden h-px w-[calc(100%-3rem)] bg-gradient-to-r from-rose/40 to-transparent lg:block"></span>
              <?php endif; ?>
              <div class="relative flex flex-col items-center text-center lg:items-start lg:text-left">
                <span class="grid size-14 place-items-center rounded-2xl border border-white/15 bg-white/5 text-rose backdrop-blur transition-all duration-300 group-hover:scale-110 group-hover:border-rose/50"><?= icon($p['icon'], 'size-6') ?></span>
                <span class="mt-4 font-display text-sm font-600 tracking-widest text-rose/70"><?= e($p['n']) ?></span>
                <h3 class="mt-1 font-display text-lg font-600 text-white"><?= e($p['title']) ?></h3>
    ...
    <section id="galeria" class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
      <div class="grid gap-8 lg:grid-cols-12 lg:items-end">
        <div class="reveal lg:col-span-6">
          ...
          <h2 class="mt-4 font-display text-4xl font-500 tracking-tight text-primary text-balance sm:text-5xl">Um ambiente feito para o seu <span class="italic font-400 text-accent">conforto</span></h2>
        </div>
        <p class="reveal text-lg leading-relaxed text-muted-foreground lg:col-span-5 lg:col-start-8" style="transition-delay:80ms">
          Consultório, recepção e equipamentos pensados para tornar a sua visita leve, segura e acolhedora.
        </p>
      </div>
    ...
    <!-- Depoimentos -->
    <section id="depoimentos" class="bg-secondary/60 py-20 lg:py-28">
      <div class="mx-auto max-w-7xl px-5 lg:px-8">
        <div class="reveal flex flex-col items-center gap-6 text-center">
          ...
          <h2 class="max-w-2xl font-display text-4xl font-500 tracking-tight text-primary text-balance sm:text-5xl">Histórias de quem <span class="italic font-400 text-accent">confia</span> no cuidado</h2>
          <div class="inline-flex items-center gap-3 rounded-full border border-border bg-background px-5 py-3 shadow-[var(--shadow-soft)]">
            <span class="flex gap-0.5 text-rose"><?php for($k=0;$k<5;$k++) echo icon('star','size-4 fill-rose'); ?></span>
        ...
        <div class="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <?php foreach ($cfg['depoimentos'] as $i => $t): ?>
            <figure class="reveal surface-card flex flex-col p-7" style="transition-delay:<?= ($i % 3) * 70 ?>ms">
              <?= icon('quote', 'size-8 text-rose/40') ?>
              <blockquote class="mt-4 flex-1 text-[0.95rem] leading-relaxed text-foreground">“<?= e($t['text']) ?>”</blockquote>
              <figcaption class="mt-6 flex min-w-0 items-center gap-3 border-t border-border pt-5">
                <span class="grid size-11 shrink-0 place-items-center rounded-full font-display text-sm font-600 text-accent-foreground" style="background: var(--gradient-rose)"><?= e($t['initials']) ?></span>
                <span class="min-w-0">
                ...
                <span class="ml-auto flex gap-0.5 text-rose"><?php for($k=0;$k<5;$k++) echo icon('star','size-3 fill-rose'); ?></span>
              </figcaption>
            </figure>
          <?php endforeach; ?>
        </div>
    ...
    <section class="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24">
      <div class="reveal relative overflow-hidden rounded-[2.75rem] px-6 py-16 text-center shadow-[var(--shadow-lift)] lg:px-16 lg:py-24" style="background: var(--gradient-deep)">
        <div aria-hidden="true" class="pointer-events-none absolute -top-24 -left-16 size-96 rounded-full bg-rose/15 blur-3xl"></div>
        ...
        <div class="relative mx-auto max-w-2xl">
          <span class="flex justify-center gap-0.5 text-rose"><?php for($k=0;$k<5;$k++) echo icon('star','size-5 fill-rose'); ?></span>
          <h2 class="mt-6 font-display text-4xl font-500 tracking-tight text-white text-balance sm:text-5xl"><?= e($cfg['cta_titulo']) ?></h2>
      ...
      <div class="overflow-hidden rounded-[2.5rem] border border-border shadow-[var(--shadow-soft)]">
        <div class="grid lg:grid-cols-[0.85fr_1.15fr]">
          <div class="reveal bg-card p-8 lg:p-12">
            ...
            <h2 class="mt-4 font-display text-3xl font-500 tracking-tight text-primary sm:text-4xl">Fácil de chegar, em <?= e($cfg['city']) ?></h2>
    ...
    <div class="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
      <div class="sm:col-span-2 lg:col-span-1">
        <div class="flex items-center gap-3">
          <img src="<?= e($cfg['logo']) ?>" alt="<?= e($cfg['clinic']) ?>" class="size-11" />
          <span class="font-display text-xl font-600"><?= e($cfg['short']) ?></span>
        </div>
        <p class="mt-4 max-w-xs text-sm leading-relaxed text-white/60">Dentista em Cidade Exemplo. Atendimento personalizado e humanizado, com estética, tecnologia e cuidado em cada detalhe.</p>
        <div class="mt-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs">
          <span class="flex gap-0.5 text-rose"><?php for($k=0;$k<5;$k++) echo icon('star','size-3 fill-rose'); ?></span>
          <span class="text-white/80"><?= e($cfg['rating']) ?> · <?= e($cfg['reviews']) ?> avaliações</span>
        </div>
        <?php if ($hasInsta): ?>
          <a href="<?= e($cfg['instagram']) ?>" target="_blank" rel="noopener noreferrer" class="mt-4 inline-flex items-center gap-2 text-sm text-white/70 transition-colors hover:text-rose">
            <?= icon('instagram', 'size-4') ?><?= e($cfg['instagram_handle'] ?? 'Instagram') ?>
          </a>
        <?php endif; ?>
      </div>
      <div>
        <h3 class="text-sm font-semibold text-white">Tratamentos</h3>
        <ul class="mt-4 space-y-2.5 text-sm text-white/60">
          <?php foreach (array_slice($cfg['especialidades'], 0, 8) as $s): ?>
            <li><a href="#tratamentos" class="transition-colors hover:text-rose"><?= e($s['title']) ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
      <div>
        <h3 class="text-sm font-semibold text-white">Contato & Endereço</h3>
        <ul class="mt-4 space-y-2.5 text-sm text-white/60">
          <li><a href="tel:<?= e($cfg['phone_raw']) ?>" class="transition-colors hover:text-rose">Telefone: <?= e($cfg['phone']) ?></a></li>
          <li><a href="<?= e($waLink) ?>" target="_blank" rel="noopener noreferrer" class="transition-colors hover:text-rose">WhatsApp: <?= e($cfg['phone']) ?></a></li>
          <?php if ($hasInsta): ?><li><a href="<?= e($cfg['instagram']) ?>" target="_blank" rel="noopener noreferrer" class="transition-colors hover:text-rose">Instagram: <?= e($cfg['instagram_handle'] ?? '') ?></a></li><?php endif; ?>
          <li class="pt-1 leading-relaxed"><?= e($cfg['address']) ?><br /><?= e($cfg['city']) ?> - <?= e($cfg['state']) ?></li>
        </ul>
      </div>
      <div>
